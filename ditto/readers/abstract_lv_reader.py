...
class AbstractLVReader(AbstractReader):
    """
    An Abstract reader that is capable of splitting a larger network into separate LV networks.  Subclass Readers must implement the various get_LV*() methods.
    """
    ...
    def parse_lv_networks(self, m: Store, show_progress: bool, **kwargs) -> list:
        ...
        for trans in tqdm(
            transformers,
            desc=f"Reading network downstream of LV transformers from {self.input_file}",
            disable=not show_progress,
        ):
            lv_network = self.parse_lv_network_from(trans)

            """ Make sure we're not duplicating any existing network names. """
            dup_count = sum([line.name == lv_network.name for line in lv_networks])
            loop_count = 2
            while dup_count > 0:  # if there are any duplicates in the list already
                new_name = (
                    lv_network.name.replace(f" #{loop_count - 1}", "")
                    + f" #{loop_count}"
                )  # drop the previously used ' #n' suffix and append the new one ' #n+1
                self.logger.warning(
                    f'Duplicate network name "{lv_network.name}" found, renaming to "{new_name}"'
                )
    ...
    def parse_lv_network_from(self, trans):
        ...
        name_counts = collections.Counter(
            [m.name for m in lv_network.models if hasattr(m, "name")]
        )
        dup_names = {
        ...
        if len(dup_names) > 0:
            self.logger.error(
                f'In network "{lv_network.name}" - found and renamed {len(dup_names)} duplicate named models : {dup_names}.'
            )
...
def get_impedance_from_matrix(impedance_matrix):
    ...
    alpha = exp((2 * pi) / 3j)
    a = matrix([[1, 1, 1], [1, alpha**2, alpha], [1, alpha, alpha**2]])
